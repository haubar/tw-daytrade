...
- [ ] 到 `/.netlify/functions/backtest-latest` 確認可讀取最新一筆回測結果（含新版高級當沖回測數據 `adv`）

回填約 6 個月約需重複執行 120 次；中斷後可從最後一個 `nextEndDate` 繼續。同一訊號日重跑會覆寫，不會重複累計。
...
**先提醒**：如果你還沒跑過第 3.5 步的 `backfill-history`，畫面上的觀察榜與回測數據可能會是空的（因為歷史累積庫還沒暖機），這是正常的，不是前端壞了。跑過 `backfill-history` 與 `backfill-backtest` 再重新整理頁面看看。

- [ ] 資料時間、大盤漲跌幅、候選檔數這些數字看起來合理嗎？
- [ ] 多方觀察榜是不是紅色（漲）、空方觀察榜是不是綠色（跌）？
- [ ] 每一列右邊的「因子解剖條」，四個顏色的分段看得清楚嗎？
- [ ] 篩選面板的股價雙把手、成交量與漲跌幅拖桿能正常拖曳嗎？股價軌道是否只有選定區間呈金色？點選價格帶（含「500 以上」）後，範圍是否正確更新？
- [ ] 套用任一篩選後按「清除篩選」，榜單檔數是否恢復；千元以上股票是否仍維持排除？
- [ ] 價格帶內的股票是否出現「獲利參考 +X 元（N 檔）」提示；500～999 元的股票不應被自動套用獲利跳檔數。
- [ ] **（本階段新增）** 檢查回測區塊是否已變為雙欄對比？左側為「基準策略（開盤買、收盤賣）」，右側為「★ 高級當沖策略（15m突破買、動態止損/止盈）」，勝率與報酬率是否正確顯示？
- [ ] **（本階段新增）** 按下方向鍵序列「上→下→左→右」叫出歷史面板，裡面的表格是否改為並列顯示「基準淨利 / 勝率」與「★ 高級淨利 / 勝率」？兩列數字是否有正確對比？
- [ ] 手機瀏覽器打開，兩欄有沒有變成單欄堆疊？

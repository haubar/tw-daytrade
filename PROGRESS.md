...
**已知未完成 / 待驗證**：
- 篩選 UI 目前只有數字輸入框，使用者原本提到「之後可以優化成滑桿」，先不做
...
- **請使用者實際看一次篩選面板的畫面**，確認排版跟操作符合預期
- TPEx 欄位對應、Blobs 讀寫細節、排程註冊、前端視覺呈現整體複查
- 持續觀察 `backfill-history` 在真實環境下的穩定度

---

## 階段 24：拖曳式篩選與當沖操作價格帶（本階段）

**目標**：把原本的數字輸入篩選改為較適合看盤操作的拖曳式介面，並納入指定的價格帶、成交量與漲跌幅門檻。

**完成事項**：
1. `FilterPanel.vue` 改為雙把手股價範圍滑桿，並提供十個價格帶的一鍵按鈕；成交量改成 100／500／1,000／5,000／10,000 張，漲跌幅改成 1／3／5／7／10% 的離散拖曳門檻。
2. 千元以上股票改為固定排除條件；即使清除使用者自行設定的條件，也不會重新出現在當沖參考榜內。
3. 修正「清除篩選」：不再替換父層傳入的 reactive 物件，而是重設既有欄位，確保畫面與計算結果同步恢復。
4. `filterWatchlist.js` 集中管理價格帶與略過檔數；正常的 0.5 元跳動價格會被連續歸入相鄰價格帶，不會產生未分類區間。每一列符合價格帶的股票會顯示「參考略過 N 檔」。

**驗證方式**：
- `npm run test`：143 個案例全數通過。

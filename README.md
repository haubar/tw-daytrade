...
**首次真實部署結果（已驗證，但發現需要進一步排查的問題）**：第一次真實請求時，`dataSourceStatus.finmindTpexInstitutional` 回報「查詢 20 檔上櫃候選，成功 0 檔」——所有請求技術上都成功（HTTP 200、FinMind 的 `body.status` 也是 200），但每一筆回傳的 `data` 都是空陣列。當時的程式碼沒有把「技術上成功但資料是空的」跟「真正失敗」分開追蹤，導致這種矛盾結果（查了 20 檔、0 成功、卻也沒有失敗紀錄）完全無法診斷。已修正 `finmind.mjs`／`scan.mjs`：現在會分別回報成功／空資料／失敗三種數量，且成功數為 0 時會附上前 3 筆的診斷細節（`debugInfo`：每檔股票實際拿到幾筆資料、有沒有錯誤訊息）。**下一次部署後，請把新的 `finmindTpexInstitutional` 內容回報，用來判斷根本原因**——最可能的解釋是免費層／目前 token 對「近期日期」的資料存取範圍有限制（跟階段 20 討論 FinMind 免費層限制時的疑慮一致），但也可能是其他原因，需要看 `debugInfo` 才能確定。
...
```bash
npm install
npm run test                # 跑全部測試（183 個案例，見 TEST_REPORT.md）

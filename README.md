...
- 最低／最高股價使用雙把手滑桿，也可以一鍵選擇操作價格帶；千元以上股票固定排除
- 最小成交量以台股慣用的「張」設定為 100、500、1,000、5,000、10,000 張以上
- 最小漲跌幅度設定為 1%、3%、5%、7%、10%（取絕對值，多方看漲幅、空方看跌幅）
- 價格帶會顯示對應的「參考略過檔數」；這是操作提示，不會改變後端排名或分數

「清除篩選」會重設使用者調整的股價／成交量／漲跌幅條件；**千元以上固定排除**仍然保留。500～999 元的股票可正常顯示，但因指定規則沒有提供該區間的略過檔數，系統不會自行推測。

...
台股全市場盤後篩選工具，用免費公開資料計算「量能異常／跳空幅度／相對大盤強弱勢／三大法人買賣超」四大因子，產生隔日觀察名單。**僅供參考，不構成投資建議，當沖有資格與風險限制，請自行評估。**
...
**首次真實部署結果（根因已找到並修正）**：第一次真實請求時，`dataSourceStatus.finmindTpexInstitutional` 回報「查詢 20 檔上櫃候選，成功 0 檔」，且 `debugInfo` 顯示查詢的股票代碼是像 `738755`、`738137` 這種 6 位數字——**這不是股票代碼，是權證代碼**。台股權證的代碼固定是 6 位數字，跟一般股票（4 位數字）／ETF（4~5 位數字）完全不同。TPEx 官方回傳的每日行情原始資料裡混雜了權證，而 `getTpexCandidateCodes` 抽出候選名單時沒有先過濾掉，導致 FinMind 查的是根本不存在的「股票」，當然查無資料。
...
- **TPEx（上櫃）欄位驗證進度**：欄位名稱本身有猜對（沒有拋出「欄位對應失敗」的例外），但部署後發現一個資料品質問題——`tpex_mainboard_daily_close_quotes` 回傳的清單混雜了大量權證（Warrant），導致「4644 檔」這種遠高於真實上櫃股票數（約 800 檔）的異常結果。已加入 `isWarrant` 過濾（見 `normalize.mjs`，依代碼位數＋名稱關鍵字判斷），TWSE／TPEx 兩邊抓取都套用。過濾後的實際檔數仍待下一次部署確認是否落回合理範圍。
- **上櫃法人資料（FinMind）尚未經過真實請求驗證**：程式碼是照官方文件格式撰寫的，見上方「上櫃法人因子的兩階段設計」說明。只對「第一輪觀察榜裡的上櫃股票」查詢，不是全市場——如果一檔上櫃股票沒有進第一輪觀察榜的前段，它的法人因子會維持中性值，不會被 FinMind 補強。目前上櫃候選數量一直是 0，正在排查是「歷史資料沒有涵蓋上櫃股票」還是「權證污染候選池排擠掉真正的股票排名」（見 PROGRESS.md 階段 31／32）。

...
//
// 這裡刻意保持嚴格的字首比對，不做模糊匹配（例如額外辨識 '⚠' 開頭）：曾經在 scan.mjs 裡
// 發現有訊息用「⚠ 全部無有效資料」「查詢失敗」這種沒有依循慣例的措辭，導致這裡把真正的
// 失敗誤判成 unknown。修法是回頭把 scan.mjs 的措辭統一改成標準的 'ok'／'失敗' 開頭
// （單一事實來源：約定寫在 scan.mjs，不是在這裡加一堆特例去猜每種可能的措辭），
// 這樣往後只要有新的資料源訊息，也會被要求遵循同一套慣例，而不是讓分類器越補越複雜。
const OK_PREFIX = 'ok';

...
//
// QA 交叉驗證：src/data/sampleScanResult.json 是前端開發時手動編造的範例資料，
...
import { screenWatchlists } from '../../netlify/functions/lib/screen.mjs';
...
const scanHandler = (await import('../../netlify/functions/scan.mjs')).default;
const response = await scanHandler(new Request('http://localhost/scan'));
const realOutput = await response.json();

// ---- 讀取純 JSON 範例資料 ----
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const sampleDataPath = path.join(__dirname, '..', '..', 'src', 'data', 'sampleScanResult.json');
...
check(
  ...
  '頂層欄位結構應該一致（sampleScanResult.json 是不是漏放或多放了某個欄位）',
